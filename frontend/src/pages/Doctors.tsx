...
const DoctorsPage: React.FC = () => {
  ...
  }, [doctors, specialtyFilter, sortBy, searchTerm]);
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="text-sm text-gray-600">
                Tìm thấy {filteredDoctors.length} chuyên gia
              </div>
            </div>
        ...
        {!loading && !error && currentDoctors.length > 0 && (
            ...
            {currentDoctors.map((doctor) => (
                <Link key={doctor.id} to={`/chuyen-gia/${doctor.id}`} className="group block">
        ...
        {!loading && !error && filteredDoctors.length > 0 && (
            <div className="flex flex-col sm:flex-row justify-between items-center mt-10 gap-4">
                <span className="text-sm text-gray-600">
                  Hiển thị {indexOfFirstDoctor + 1}-{Math.min(indexOfLastDoctor, filteredDoctors.length)} trên {filteredDoctors.length} kết quả
                </span>
