...
const getStartOfWeek = (date: Date): Date => {
  ...
  const diff = d.getDate() - day; // Sunday as the first day
  d.setHours(0, 0, 0, 0); // Reset time to midnight
  const result = new Date(d.setDate(diff));

  console.log("getStartOfWeek:", { inputDate: date, day, diff, result });
...
const ServiceDetailPage: React.FC = () => {
  ...
  const groupAvailabilitiesByDate = () => {
    ...
    availabilities.forEach((availability) => {
      ...
      const isoDate = availability.available_date;
      const date = new Date(isoDate);
      const dateStr = formatDate(date); // This will give us YYYY-MM-DD format
