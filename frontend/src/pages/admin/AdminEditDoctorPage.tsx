...
import AdminDoctorForm, { DoctorFormData } from '../../components/AdminDoctorForm';
import { LuArrowLeft } from 'react-icons/lu';
import doctorService, { Doctor, DoctorUpdatePayload } from '../../api/doctorService';

const AdminEditDoctorPage: React.FC = () => {
  const { doctorId } = useParams<{ doctorId: string }>();
  const navigate = useNavigate();
  
  const [initialData, setInitialData] = useState<Partial<DoctorFormData> | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDoctor = async () => {
      ...
      try {
        ...
        // Map Doctor to doctor FormData for the form
        const formData: Partial<DoctorFormData> = {
            id: data.id,
  ...
  const handleEditDoctor = async (formData: DoctorFormData) => {
    if (!doctorId) {
    ...
    console.log('Submitting updated doctor data to API:', apiPayload);
    setIsSubmitting(true);
    setSubmitError(null);

    try {
      // Update basic doctor information
      await doctorService.update(numericDoctorId, apiPayload);
      ...
      const currentData = await doctorService.getById(numericDoctorId);
      ...
      if (qualifications && qualifications.length > 0) {
        ...
        for (const qualification of qualifications) {
          if (qualification.degree && qualification.major && qualification.completion_year && qualification.institution) {
            try {
              await doctorService.addQualification(numericDoctorId, {
        ...
        for (const experience of experiences) {
          if (experience.position && experience.start_date && experience.workplace) {
            try {
              ...
              await doctorService.addExperience(numericDoctorId, validExperience);
            } catch (expError) {
  ...
  if (fetchError) {
    return (
      <div className="p-6">
        <button onClick={() => navigate('/admin/doctors')} className="flex items-center text-sm text-gray-600 hover:text-gray-900 mb-4">
