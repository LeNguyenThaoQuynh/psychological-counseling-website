...
import { LuPlus, LuSearch, LuSlidersHorizontal, LuFilePenLine, LuTrash2, LuCircleUserRound, LuEye } from 'react-icons/lu';
import { useNavigate, Link } from 'react-router-dom';
import doctorService, { Doctor } from '../../api/doctorService'; // Import Doctor type and service

// No longer using mock doctor interface or data
...
const AdminDoctorsPage: React.FC = () => {
  ...
  const fetchDoctors = useCallback(async () => {
    ...
    try {
      ...
      if (searchTerm.trim() !== '') {
        fetchedDoctors = await doctorService.search(searchTerm.trim());
      } else {
        fetchedDoctors = await doctorService.getAll();
      }
      setDoctors(fetchedDoctors);
      setTotalDoctors(fetchedDoctors.length); // Assuming API doesn't provide total for pagination yet
    } catch (err: any) {
  ...
  const handleDeleteDoctor = async (doctorId: number, doctorName?: string) => {
    const confirmMessage = doctorName 
      ? `Bạn có chắc chắn muốn xóa chuyên gia "${doctorName}" (ID: ${doctorId}) không?`
      : `Bạn có chắc chắn muốn xóa chuyên gia ID: ${doctorId} không?`;

    if (window.confirm(confirmMessage)) {
      try {
        await doctorService.delete(doctorId);
        setDoctors(prevDoctors => prevDoctors.filter(doctor => doctor.id !== doctorId));
        setTotalDoctors(prevTotal => prevTotal - 1);
        alert(`Đã xóa chuyên gia ${doctorName || `ID ${doctorId}`}.`);
      } catch (err: any) {
  ...
  const filteredAndSortedDoctors = useMemo(() => {
    ...
    return result;
  }, [doctors, searchTerm, specialtyFilter, sortConfig]);

  // // Get unique specialties for filter
  // const specialties = useMemo(() => {
  //   const uniqueSpecialties = new Set(doctor.map(doctor => doctor.specialty).filter(Boolean));
          ...
          {paginatedDoctors.map((doctor) => (
            <div key={doctor.id} className="relative group">
              <DoctorCard 
                ...
                imageUrl={doctor.profile_picture || 'https://via.placeholder.com/64x64.png?text=Avatar'}
      ...
      {totalDoctors > 0 && (
        <div className="flex justify-between items-center mt-4 px-4 py-3 border-t border-gray-300">
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span>Hiển thị {(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredAndSortedDoctors.length)} trên {filteredAndSortedDoctors.length} chuyên gia</span>
          </div>
