...
import { LuArrowLeft, LuLoader, LuCalendar, LuClock, LuUser, LuBanknote, LuClipboard, LuCheck, LuX, LuChevronDown, LuChevronUp } from 'react-icons/lu';
import orderService, { Order, OrderStatus, Appointment } from '../../api/orderService';
...
const AdminConsultationDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || 'Không thể tải thông tin đơn tư vấn');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleOrderStatusUpdate = async (status: OrderStatus) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Không thể cập nhật trạng thái đơn tư vấn');
    } finally {
      setUpdatingOrder(false);
    }
  };

  const handleAppointmentStatusUpdate = async (appointmentId: number, status: string) => {
    try {
      ...
      await axiosClient.put(`/appointments/${appointmentId}/status`, { status });
      ...
      setError(err.response?.data?.message || 'Không thể cập nhật trạng thái buổi tư vấn');
  ...
  if (error || !order) {
    return (
      <div className="p-6">
        <Link to="/admin/consultations" className="inline-flex items-center text-blue-600 hover:text-blue-700 mb-4">
          <LuArrowLeft className="mr-2 h-4 w-4" /> Quay lại danh sách
        </Link>
        <div className="bg-red-50 border border-red-200 p-4 rounded-lg">
          <p className="text-red-600">{error || 'Không tìm thấy thông tin đơn tư vấn'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-background min-h-screen rounded-[16px]">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-600">
          <Link to="/admin/consultations" className="hover:text-blue-600">Ca tham vấn</Link>
          <span>/</span>
          ...
          <div className="border-t border-gray-200 p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Khách hàng</h3>
                <div className="flex items-center space-x-2">
                  <LuUser className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-900 font-medium">{order.client_name}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <LuCalendar className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600 text-sm">
                    {format(new Date(order.order_created_at || ''), 'dd/MM/yyyy HH:mm')}
                  </span>
                </div>
              </div>
              ...
              {/* Service Info */}
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Dịch vụ</h3>
                <div className="flex items-center space-x-2">
                  <LuClipboard className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-900 font-medium">{order.service_name}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <LuUser className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600 text-sm">{order.doctor_name}</span>
                </div>
              </div>

              {/* Progress Info */}
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Tiến độ</h3>
                <div className="flex items-center space-x-2">
                  <LuClock className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-900 font-medium">
                    {order.completed_sessions || 0}/{order.number_of_sessions} buổi
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${((order.completed_sessions || 0) / order.number_of_sessions) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              {/* Payment Info */}
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide">Thanh toán</h3>
                <div className="flex items-center space-x-2">
            ...
            {order.notes && (
              <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <h4 className="text-sm font-medium text-blue-900 mb-2">Ghi chú đơn hàng</h4>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {currentAppointments.map((appointment) => (
                    ...
                    >
                      {/* Card Header */}
                      <div className={`p-4 border-l-4 ${getStatusColor(appointment.status).replace('text-', 'border-').split(' ')[2]}`}>
                        ...
                        {/* Expandable Content */}
                        {(appointment.notes || appointment.completion_notes) && (
                          <button
                          ...
                          <div className="mt-3 space-y-2">
                            {appointment.notes && (
                              <div className="bg-yellow-50 border border-yellow-200 p-2 rounded text-xs">
                                ...
                                <p className="text-yellow-700 mt-1">{appointment.notes}</p>
                              </div>
                            )}
                            {appointment.completion_notes && (
                              <div className="bg-green-50 border border-green-200 p-2 rounded text-xs">
                                <span className="font-medium text-green-800">Hoàn thành:</span>
                                <p className="text-green-700 mt-1">{appointment.completion_notes}</p>
                              </div>
                            )}
                          </div>
                        )}
