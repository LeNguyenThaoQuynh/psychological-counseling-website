...
const AdminEditServicePage: React.FC = () => {
  ...
  const [initialAssignedDoctors, setInitialAssignedDoctors] = useState<DoctorOption[]>([]);
    ...
    medicalServiceService.getById(idFromParam)
      .then(serviceApiResponse => {
        if (serviceApiResponse && typeof serviceApiResponse.id === 'number') {
          ...
          // Set initial assigned Doctors
          if (serviceApiResponse.doctor_details && serviceApiResponse.doctor_details.length > 0) {
            const assignedDoctors = serviceApiResponse.doctor_details.map(doctor => ({
              value: doctor.id,
              label: doctor.name
            }));
            setInitialAssignedDoctors(assignedDoctors);
          }
        } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <AdminServiceForm
        ...
        initialAssignedDoctors={initialAssignedDoctors}
      />
      {isSubmitting && (
