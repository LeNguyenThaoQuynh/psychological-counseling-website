...
import { LuSearch, LuChevronDown, LuSlidersHorizontal, LuStar, LuCircleUserRound } from 'react-icons/lu';
...
const AdminReviewsPage: React.FC = () => {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const response = await reviewService.getAll(currentPage, limit, searchTerm);
  ...
  const formatDate = (dateString: string) => {
    return format(new Date(dateString), "EEEE, 'ngày' d 'tháng' M yyyy, HH:mm", { locale: vi });
  };
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 divide-y divide-gray-100">
        {loading ? (
          ...
          sortedReviews.map((review) => (
            <div key={review.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-gray-50">
              ...
              <div className="flex items-center gap-3 flex-grow min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{review.customer_name}</p>
                  <p className="text-xs text-gray-500 truncate">{formatDate(review.date_time)}</p>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 text-xs text-gray-600 w-full sm:w-auto flex-shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                <div className="min-w-[120px]">
                  ...
                  <p className="font-medium text-gray-700 truncate">{review.service_name}</p>
                </div>
                <div className="min-w-[120px]">
                  <p className="text-gray-400 mb-0.5">Chuyên gia</p>
                  <p className="font-medium text-gray-700 truncate">{review.expert_name}</p>
                </div>
                <div>
          ...
          <p className="p-6 text-center text-gray-500">Không có đánh giá nào.</p>
        )}
      </div>
      ...
      {totalPages > 0 && (
        <div className="flex justify-between items-center mt-6 px-4 py-3 bg-white rounded-lg shadow-sm border border-gray-100">
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span>Hiển thị {(currentPage - 1) * limit + 1}-{Math.min(currentPage * limit, total)} trên {total} đánh giá</span>
          </div>
