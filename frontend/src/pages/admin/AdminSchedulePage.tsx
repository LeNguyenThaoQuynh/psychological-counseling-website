...
import { LuChevronLeft, LuChevronRight, LuClock, LuUser, LuMapPin } from 'react-icons/lu';
...
const AdminSchedulePage: React.FC = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await axiosClient.get<Appointment[]>(`/appointments/week?start=${startDate}&end=${endDate}`);
    ...
    return appointments
      ...
      .map(apt => ({
        ...
        status: apt.status === 'confirmed' ? 'confirmed' : 
                apt.status === 'pending' ? 'pending' : 'consultation',
  ...
  const getUniqueServicesFromAppointments = () => {
    const uniqueServiceNames = [...new Set(appointments.map(apt => apt.service_name).filter(Boolean))];
    ...
    <div className="p-6 bg-white min-h-screen rounded-[16px]">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center space-x-3 text-sm">
              {uniqueServices.map(service => (
                <div key={service.name} className="flex items-center space-x-1">
                  <div className={`w-3 h-3 rounded ${service.color.split(' ')[0]}`}></div>
                  <span className="text-gray-800">{service.name}</span>
                </div>
              ))}
              {uniqueServices.length === 0 && (
                <span className="text-gray-500 italic">Chưa có dịch vụ nào</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[16px] shadow-sm overflow-hidden">
          ...
          {/* Time Slots */}
          {timeSlots.map(time => (
            <div key={time} className="grid grid-cols-[50px_repeat(7,_1fr)] border-b border-gray-300 min-h-[100px]">
              {/* Time Column */}
              ...
              {weekDates.map((date, dayIndex) => {
                ...
                return (
                  <div key={dayIndex} className="p-2 border-r border-gray-100 relative">
                    {events.map(event => (
                      <div
                        key={event.id}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-medium text-xs">{event.room}</span>
                        ...
                        {/* Patient Info */}
                        <div className="font-medium text-gray-900 mb-1">
                          <span className="text-green-700">KH:</span> {event.client}
                        </div>
                        ...
                        <div className="mt-1">
                          <span className={`inline-block w-2 h-2 rounded-full ${
                            ...
                            event.status === 'pending' ? 'bg-yellow-300' : 'bg-blue-500'
                          }`}></span>
                        </div>
                      </div>
                    ))}
