...
const AdminDashboardPage: React.FC = () => {
  ...
  const [invoiceStats, setInvoiceStats] = useState<TotalInvoiceStats | null>(null);
  const [patientAgeData, setPatientAgeData] = useState<PatientAgeData[]>([]);
  const [revenueData, setRevenueData] = useState<RevenueChartData[]>([]);
  const [departmentData, setDepartmentData] = useState<PatientOverviewByDepartments | null>(null);
  const [paymentDistribution, setPaymentDistribution] = useState<PaymentStatusDistribution | null>(null);
...
// Luôn đảm bảo 3 lớp
const allAgeGroups: ('Child' | 'Adult' | 'Elderly')[] = ['Child', 'Adult', 'Elderly'];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex justify-center items-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600 font-medium">Đang tải dashboard...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex justify-center items-center">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md mx-auto">
          ...
          <h3 className="text-xl font-semibold text-gray-800 text-center mb-2">Lỗi tải dữ liệu</h3>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-[16px] p-6 h-32 border border-gray-100 shadow-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-purple-700">Doanh thu</span>
              </div>
              <h3 className="text-lg font-bold text-gray-800">{formatCurrency(stats?.monthlyRevenue || 0)}</h3>
            ...
            <div className="bg-white rounded-[16px] p-6 h-32 border border-gray-100 shadow-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-green-700">Số khách hàng</span>
              </div>
              <h3 className="text-lg font-bold text-gray-800">{stats?.totalClients.toLocaleString()}</h3>
            ...
            <div className="bg-white rounded-[16px] p-6 h-32 border border-gray-100 shadow-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-orange-700">Gói dịch vụ</span>
              </div>
              <h3 className="text-lg font-bold text-gray-800">{stats?.totalServices}</h3>
            ...
            <div className="bg-white rounded-[16px] p-6 h-32 border border-gray-100 shadow-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-blue-700">Lịch hôm nay</span>
              </div>
              <h3 className="text-lg font-bold text-gray-800">{stats?.appointmentsToday}</h3>
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6">
              ...
              {activities.length > 0 ? (
                activities.map((activity, index) => (
                  <div key={index} className="flex items-start space-x-3">
                    <div className={`p-2 rounded-[16px] ${getActivityColor(activity.type)}`}>
                      {getActivityIcon(activity.type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 mb-1">{getActivityText(activity)}</p>
          ...
          <div className="bg-white rounded-[16px] shadow-md border border-gray-100 overflow-hidden ">
            ...
            <div className="p-4 max-h-64 overflow-y-auto space-y-3">
              {schedule.length > 0 ? (
                schedule.slice(0, 6).map((appointment) => (
                  <div key={appointment.id} className="flex items-center border border-gray-300 space-x-3 p-3 rounded-[16px] hover:bg-gray-50 transition-all duration-200">
                    <div className="w-3 h-3 rounded-full bg-green-400"></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1 ">
                        <p className="text-sm font-semibold">{appointment.doctor_name}</p>
                        <span className="text-xs font-semibold bg-red-100 px-1.5 py-1 rounded-full">
                          Room {appointment.session_number.toString().padStart(3, '0')}
                        </span>
                      </div>
                      <p className="text-xs mb-1">{appointment.client_name}</p>
                      <p className="text-xs font-medium">
                        {formatTime(appointment.scheduled_at)} - {appointment.end_time || '10:30AM'}
                      </p>
                    </div>
                  </div>
                ))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Khách hàng theo dịch vụ</h2>
              <p className="text-sm text-gray-600">
                Tổng: {departmentData?.totalPatients?.toLocaleString() || '0'} khách hàng
              </p>
            </div>
            <div className="h-80">
              {departmentData?.departments && departmentData.departments.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">Chưa có dữ liệu khách hàng theo dịch vụ</p>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2">
                {departmentData.departments.map((dept, index) => (
                  <div key={dept.department} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-800">{dept.percentage}%</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Đánh giá dịch vụ</h2>
              <p className="text-sm text-gray-600">Rating trung bình các dịch vụ</p>
            </div>
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {serviceRatings && serviceRatings.length > 0 ? (
                serviceRatings.slice(0, 8).map((service, index) => (
                  <div key={service.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                    <div className="flex-1">
                      <h4 className="font-medium text-gray-800 text-sm">{service.service_name}</h4>
                      <div className="flex items-center space-x-2 mt-1">
                        <div className="flex items-center">
                          {[...Array(5)].map((_, i) => (
                            <LuStar 
                              key={i} 
                              className={`h-3 w-3 ${i < Math.round(service.average_rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} 
                            />
                          ))}
                        </div>
                        <span className="text-xs text-gray-600">
                          {service.average_rating.toFixed(1)} ({service.total_reviews} đánh giá)
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-800">{formatCurrency(service.price)}</p>
                      <p className="text-xs text-gray-500">{service.total_patients} khách hàng</p>
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500">Chưa có dữ liệu đánh giá dịch vụ</p>
                </div>
              )}
            </div>
          </div>
