...
import { LuDollarSign, LuImagePlus, LuUserPlus, LuHash, LuFolderPen } from 'react-icons/lu';
import Select from 'react-select';
import SunEditor from 'suneditor-react';
import 'suneditor/dist/css/suneditor.min.css';
import uploadFileApi from '../api/uploadFileApi'; // Assuming this API is suitable
// Consider using a Select component library like react-select for the doctor dropdown
// import Select from 'react-select'; 
...
interface AdminServiceFormProps {
  ...
  initialAssignedDoctors?: DoctorOption[];
}
...
const AdminServiceForm: React.FC<AdminServiceFormProps> = ({ 
  ...
  const [selectedDoctors, setSelectedDoctors] = useState<DoctorOption[]>(initialAssignedDoctors);
  ...
  useEffect(() => {
    if (initialAssignedDoctors?.length > 0) {
      setSelectedDoctors(initialAssignedDoctors);
    }
  }, [initialAssignedDoctors]);
  ...
  const handleDoctorChange = (selectedOptions: readonly DoctorOption[]) => {
  ...
  const handleFormSubmit = (data: ServiceFormData) => {
    const payload: ServiceFormData = {
      ...
      doctor_ids: selectedDoctors.map(doctor => doctor.value)
    };
