...
const appointmentController = {
  ...
  completeAppointment: async (req, res) => {
    try {
      ...
      const [appointments] = await connection.promise().query(
        'SELECT order_id, status FROM appointments WHERE id = ?',
        [id]
      );
      ...
      console.log(`Completed appointment ${id} and updated completed_sessions for order ${orderId}`);
  ...
  updateAppointmentStatus: async (req, res) => {
    try {
      ...
      const [currentAppointment] = await connection.promise().query(
        'SELECT order_id, status FROM appointments WHERE id = ?',
        [id]
      );
      ...
      if (status === 'completed' && oldStatus !== 'completed') {
        ...
        console.log(`Updated completed_sessions for order ${orderId} after appointment ${id} completion`);
      }
      // If appointment status changed from completed to something else, update count
        ...
        console.log(`Updated completed_sessions for order ${orderId} after appointment ${id} status change`);
      }
  ...
  getAppointmentsByWeek: async (req, res) => {
    try {
      ...
      const [appointments] = await connection.promise().query(query, [start, end]);
      res.json(appointments);
    } catch (error) {
