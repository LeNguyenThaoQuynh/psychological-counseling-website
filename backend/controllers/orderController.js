...
exports.createOrder = async (req, res) => {
  try {
    ...
    if (!client_id || !doctor_id || !service_id || !number_of_sessions || !amount) {
    ...
    const [services] = await connection.promise().query(
      'SELECT number_of_sessions FROM services WHERE id = ?',
      [service_id]
    );
    ...
    const orderData = {
      ...
      payment_status,
      status: 'pending',
      availability_ids: availability_ids ? JSON.stringify(availability_ids) : null, // Store as JSON
      created_at: new Date(),
      updated_at: new Date()
    };

    const [result] = await connection.promise().query('INSERT INTO orders SET ?', orderData);
    const orderId = result.insertId;

    // If availability_ids are provided, create appointments immediately
    if (availability_ids && Array.isArray(availability_ids)) {
      console.log(`Creating appointments for order ${orderId} with specific availability slots`);
      ...
      for (let i = 0; i < availability_ids.length; i++) {
        ...
        if (availability.length > 0) {
          ...
          const time = start_time.length === 5 ? start_time + ':00' : start_time;
      ...
      console.log(`Successfully created ${availability_ids.length} appointments for order ${orderId}`);
    }
    
    res.status(201).json({
      message: availability_ids && availability_ids.length > 0 
...
exports.getOrderById = async (req, res) => {
  ...
  try {
    ...
    `;
    const [appointments] = await connection.promise().query(appointmentsQuery, [id]);
...
exports.updateOrder = async (req, res) => {
  const { id } = req.params;
  const { status, notes, payment_method, payment_status, paid_at, number_of_sessions, amount } = req.body;
  ...
  try {
    const [existingOrders] = await connection.promise().query('SELECT id FROM orders WHERE id = ?', [id]);
    ...
    if (payment_method !== undefined) fieldsToUpdate.payment_method = payment_method;
    if (payment_status !== undefined) fieldsToUpdate.payment_status = payment_status;
    if (paid_at !== undefined) fieldsToUpdate.paid_at = paid_at ? new Date(paid_at) : null;
    if (number_of_sessions !== undefined) {
      if (isNaN(parseInt(number_of_sessions)) || parseInt(number_of_sessions) <= 0) {
         return res.status(400).json({ message: 'Number of sessions must be a positive integer if provided.' });
      }
      fieldsToUpdate.number_of_sessions = parseInt(number_of_sessions);
    }
    if (amount !== undefined) {
      if (isNaN(parseFloat(amount)) || parseFloat(amount) < 0) {
        return res.status(400).json({ message: 'Amount must be a valid non-negative number if provided.' });
      }
    ...
    await connection.promise().query('UPDATE orders SET ? WHERE id = ?', [fieldsToUpdate, id]);
...
exports.deleteOrder = async (req, res) => {
  const { id } = req.params;
  try {
    const [result] = await connection.promise().query('DELETE FROM orders WHERE id = ?', [id]);
...
exports.updateStatus = async (req, res) => {
  ...
  try {
    ...
    const [currentOrder] = await connection.promise().query(
      'SELECT status, number_of_sessions FROM orders WHERE id = ?',
      [id]
    );
    ...
    if (oldStatus === 'pending' && status === 'confirmed') {
      ...
      const [existingAppointments] = await connection.promise().query(
        'SELECT COUNT(*) as count FROM appointments WHERE order_id = ?',
        [id]
      );

      if (existingAppointments[0].count === 0) {
        console.log(`Creating ${numberOfSessions} appointments for order ${id}`);
        
        // Create appointments for each session
        for (let sessionNumber = 1; sessionNumber <= numberOfSessions; sessionNumber++) {
          ...
          scheduledDate.setDate(scheduledDate.getDate() + (sessionNumber - 1) * 7); // Then add weeks for each session
        ...
        console.log(`Successfully created ${numberOfSessions} appointments for order ${id}`);
      } else {
        console.log(`Order ${id} already has appointments, skipping automatic creation`);
      }
    }
